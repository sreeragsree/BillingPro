...
$(document).on("click", ".delete-batch", function (e) {
    ...
    let batId = $(this).data("bat-id");

    if (confirm("Do you want to delete this batch?")) {

        $.ajax({
            type: 'POST',
            url: base_url + "items/deleteItemBatch",
            data: {
                pro_id: proId,
                batch_id: batId,
                // pass CSRF properly
                [$("meta[name=csrf-token-name]").attr("content")]:
                    $("meta[name=csrf-token-value]").attr("content")
            },
...
$('#saveBatchEdit').on("click", function (e) {

    var base_url = $("#base_url").val();

    e.preventDefault();
    data = new FormData($('#batch_edit_form')[0]);//form name
    if (!xss_validation(data)) {
        return false;
    }

    $("#saveBatchEdit").attr('disabled', true);
    $.ajax({
        type: 'POST',
        url: base_url + "items/editBatchRecordModal",
        data: data,
        cache: false,
        contentType: false,
        processData: false,
        success: function (result) {
            if (result == "success") {
                toastr["success"]("Success ! Record updated successfully!");
            ...
            $("#saveBatchEdit").attr('disabled', false);
        }
    });
