...
// batch creating using modal by manoj
$(".add_batch").on("click", function (e) {
  ...
  e.preventDefault();
  formData = new FormData($('#batch-form')[0]);//form name

  var pro_id = formData.get("pro_id"); // Correct way to get FormData values
  var row_id = formData.get("row_id");

  /*Check XSS Code*/
  if (!xss_validation(formData)) { return false; }
  ...
  $.ajax({
    type: 'POST',
    url: base_url + 'purchase/newbatch?js_store_id=' + $("#store_id").val(),
    data: formData,
    cache: false,
    contentType: false,
    processData: false,
    success: function (result) {
      ...
      if (data.result == "success") {
        ...
        var newOption = '<option value=' + data.id + ' selected>' + data.batch_no + '</option>';
        $('#batch_id_' + row_id).append(newOption).trigger('change');
...
function get_all_batch_and_update(pro_id) {
  ...
  // $('#batch-modal').modal('toggle');
  // var newOption = '<option value=' + data.id + ' selected>' + data.batch_no + '</option>';
  // $('#batch_id'+row_id).append(newOption).trigger('change');
  // //$("#amount").val(data.advance);
  // $('#batch-form')[0].reset();
  // toastr["success"]("New batch Added!!");
  // success.currentTime = 0;
  // success.play();

}

// batch creating using modal by manoj close
