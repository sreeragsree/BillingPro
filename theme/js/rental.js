...
$('#save,#update').on("click", function (e) {
    ...
    for (var n = 1; n <= rowcount; n++) {
        if ($("#td_data_" + n + "_3").val() != null && $("#td_data_" + n + "_3").val() != '' && $("#td_data_" + n + "_3").val() != '0') {
            flag1 = true;
        }
        // check batch too herex
        // if ($("#tr_batch_id_" + n + "_111").val()) {
        //     batchFlag = true;
        // }

        // if ($("#tr_batch_id_" + n + "_111").val() != '' && $("#tr_batch_id_" + n + "_111").val() != null ) {
        //   batchFlag = true;
        // }
    }

    if (batchFlag == false) {
        toastr["warning"]("Please Select Item batch to proceed");
        return;
    }
    ...
    e.preventDefault();
    data = new FormData($('#rental-form')[0]);//form name
    /*Check XSS Code*/
    ...
    $.ajax({
        type: 'POST',
        url: base_url + 'rental/sales_save_and_update?command=' + this_id + '&rowcount=' + rowcount + '&tot_subtotal_amt=' + tot_subtotal_amt + '&tot_discount_to_all_amt=' + tot_discount_to_all_amt + '&tot_round_off_amt=' + tot_round_off_amt + '&tot_total_amt=' + tot_total_amt + "&other_charges_amt=" + other_charges_amt + "&coupon_discount_amt=" + coupon_discount_amt,
        data: data,
        cache: false,
        contentType: false,
        processData: false,
        success: function (result) {
            ...
            console.log("Rental form submit : " + result[0]);

            if (result[0] == "success") {
                location.href = base_url + "rental/invoice/" + result[1];
            }
...
$("#item_search").autocomplete({
    source: function (data, cb) {
        $.ajax({
            autoFocus: true,
            url: $("#base_url").val() + 'rental_items/get_json_items_details',
            method: 'GET',
            dataType: 'json',
            ...
            success: function (res) {
                ...
                if (res.length) {
                    result = $.map(res, function (el) {
                        ...
                        qty_ = ' ---[Qty:' + el.stock + '] --- ';
                        rental_price = (el.rental_price != '') ? ' --- [Rental Price:' + el.rental_price + '] ' : '';
                        rac_no = ' --- [Rack No :' + el.rac_no + '] --- ';


                        return {
                            label: el.item_code + " ----- " + el.label + "( " + el.color + " - " + el.size + " )"  + " ----- " + rac_no + qty_ + rental_price,
                            value: '',
                            id: el.id,
                            item_name: el.value,
                            stock: el.stock,
...
function return_row_with_data(item_id) {
    ...
    $.post(base_url + "rental/return_row_with_data/" + rowcount + "/" + item_id, { customer_id: customer_id, warehouse_id: warehouse_id }, function (result) {

        console.log(" return row with data : "+result);
...
function pay_now(sales_id) {
    var base_url = $("#base_url").val();
    $.post(base_url + 'rental/show_pay_now_modal', { sales_id: sales_id }, function (result) {
...
function view_payments(sales_id) {
    var base_url = $("#base_url").val();
    $.post(base_url + 'rental/view_payments_modal', { sales_id: sales_id }, function (result) {
...
function save_payment(sales_id) {
    ...
    $.post(base_url + 'rental/save_payment', { cheque_number: cheque_number, cheque_period: cheque_period, allow_tot_advance: allow_tot_advance, customer_id: customer_id, account_id: account_id, sales_id: sales_id, payment_type: payment_type, amount: amount, payment_date: payment_date, payment_note: payment_note }, function (result) {
        result = result;
        //alert(result);return;
        if (result == "success") {
            $('#pay_now').modal('toggle');
