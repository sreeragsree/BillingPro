...
$('#save,#update').on("click", function (e) {
  ...
  function check_field(id) {
    ...
    console.log('check feild is exce......'+id);
  ...
  // check all price feilds 

  // check all price feilds close 
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      console.log("Purchase save response : " + result);
...
selectInitial :true,*/
      ...
      beforeSend: function () {
        ...
        console.log("Search value :"+data.term);
...
function return_row_with_data(item_id,batch_id) {
  ...
  // adding searched item to table tbody
  ...
  var base_url = $("#base_url").val();
  var rowcount = $("#hidden_rowcount").val();
  console.log("batch id inside return_row_with_data : " + batch_id);
  var batch_id = isNaN(parseInt(batch_id)) ? 0 : parseInt(batch_id);
  $.post(base_url + "purchase/return_row_with_data/" + rowcount + "/" + item_id, {
    batch_id: batch_id,
  }, function (result) {
    //alert(result);
