      ...
      success: function (res) {
        ...
        if (res.length) {
          result = $.map(res, function (el) {
            ...
            sales_price = (el.sales_price != '') ? ' --- [Sales Price:' + el.sales_price + '] --- ' : '';

            rac_no = ' --- [Rack No :' + el.rac_no + '] --- ';

            return {
              label: el.item_code + " ----- " + el.label + " ----- " + rac_no + qty_ + sales_price,
              value: '',
              id: el.id,
              item_name: el.value,
              stock: el.stock,
  ...
  select: function (e, ui) {
   ...
   if(restrict_quantity(item_id)){
    ...
    if (typeof ui.content != 'undefined') {
      ...
      console.log("BatchCode form first is : " + batch_id);
    }
    else {
      ...
      console.log("BatchCode form second is : " + batch_id);
    }
    ...
    if (service_bit == 1) {
      ...
      if (restrict_quantity(item_id)) {
        // for auto selection from barcode-batchid
        return_row_with_data(item_id,batch_id);
      }
    }
    $("#item_search").val('');
...
function return_row_with_data(item_id,batch_id) {
  ...
  $.post(base_url + "sales/return_row_with_data/" + rowcount + "/" + item_id,
