...
class Rented_model extends CI_Model
{
	//Datatable start
	var $table = 'db_rented as a';
	...
	private function _get_datatables_query()
	{
		...
		if ($sales_from_date != '1970-01-01') {
			$this->db->where("a.rental_date>=", $sales_from_date);
		}
		if ($sales_to_date != '1970-01-01') {
			$this->db->where("a.rental_date<=", $sales_to_date);
		}
	...
	public function verify_save_and_update()
	{
		...
		validate_package_offers('max_invoices', 'db_rented');
		//END
		...
		if (empty(trim($count_id))) {
			echo "Invoice Number Should be not be Empty!";
			exit;
		} else {
			if (!is_numeric($count_id)) {
				echo "Invoice Number Should be Numerical!";
				exit;
			}
		}
		...
		//Verify Sales Code
		$this->db->where("rental_code", $init_code . $count_id);
		if ($command == 'update') {
			$this->db->where("id<>", $sales_id);
		}
		$this->db->from('db_rented');
		//echo $this->db->get_compiled_select();exit;
		...
		if ($count > 0) {
			...
			if ($autosynch_sales_code) {
				$count_id = autosynch_rental_code();
			} else {
				echo "Rental Code already exist";
				exit;
			}
		}
		...
		$sales_entry_init = array(
			'init_code' 				=> $init_code,
			'count_id' 					=> $count_id,
			'rental_code' 				=> $init_code . $count_id, //get_init_code('sales'),
			/*Coupon disocunt amt*/
			// 'coupon_id' 				=> $customer_coupon_id,
			// 'coupon_amt' 				=> $coupon_discount_amt,
			'rental_note' 				=> trim($invoice_terms),
		);

		if ($command == 'save') { //Create sales code unique if first time entry

			$this->db->query("ALTER TABLE db_rented AUTO_INCREMENT = 1");

			$sales_entry = array(

				//'count_id' 					=> get_count_id('db_sales'),  
				// 'reference_no' 				=> $reference_no,
				'rental_date' 				=> $sales_date,
			...
			$q1 = $this->db->insert('db_rented', array_merge($sales_entry, $sales_entry_init));
		...
		for ($i = 1; $i <= $rowcount; $i++) {
			...
			if (isset($_REQUEST['tr_item_id_' . $i]) && !empty($_REQUEST['tr_item_id_' . $i])) {
				...
				$item_details = get_rental_item_details($item_id);
				...
				$current_stock_of_item = total_available_qty_rental_items_of_store($warehouse_id, null, $item_id);
				...
				$q2 = $this->db->insert('db_renteditems', $salesitems_entry);

				//UPDATE itemS QUANTITY IN itemS TABLE
				$q6 = $this->update_rental_items_quantity($item_id, $sales_qty, 'decrease');
		...
		if ($amount > 0 && !empty($payment_type)) {
			...
			$payment_code = get_init_code('rental_payment');
			$salespayments_entry = array(
				'payment_code' 		=> $payment_code,
				'count_id'	  		=> get_count_id('db_rentalpayments'),
				'rental_id' 			=> $sales_id,
			...
			$q3 = $this->db->insert('db_rentalpayments', $salespayments_entry);

			//Set the payment to specified account for rental
			if (!empty($account_id)) {
				//ACCOUNT INSERT
		...
		$q10 = $this->update_rental_payment_status($sales_id, $customer_id);
		if ($q10 != 1) {
			return "failed";
		}
	...
	public function inspect_and_update_stock($rent_id)
	{
		$q12 = $this->db->query("select * from db_renteditems where rent_id=" . $rent_id);
		if ($q12->num_rows() > 0) {
			foreach ($q12->result() as $res12) {
				if (!$this->update_rental_items_quantity($res12->item_id,$res12->rent_qty, 'increase')) {
					return false;
				}
			}
			$q13 = $this->db->query("update db_rented set returned_flag = 1 where id=" . $rent_id);
		}
		else
		{
			return 'failed';
	...
	public function return_rented_item($rent_id)
	{
		...
		if ($q12->num_rows() > 0) {
			foreach ($q12->result() as $res12) {
				$this->db->query("UPDATE db_renteditems SET rent_status = 'Returned', returned_date = '$r_date' WHERE id = " . $res12->id);
			}
			$q13 = $this->db->query("UPDATE db_rented SET rental_status = 'Returned', returned_date = '$r_date' WHERE id = " . $rent_id);
		}
		else
		{
			return 'failed';
	...
	// update rental quantity in rental items table
	function update_rental_items_quantity($item_id, $sales_qty, $type)
	{
		...
		if ($type == 'decrease') {
			$newStock = $current_stock_of_item - $sales_qty;
		} else {
			$newStock = $current_stock_of_item + $sales_qty;
		}
		$this->db->set("stock", $newStock);
		$this->db->where("id", $item_id);
		$q7 = $this->db->update("db_rentalitems");
	...
	function update_rental_payment_status_by_sales_id($sales_id, $customer_id)
	{
		$q8 = $this->db->query("select COALESCE(SUM(payment),0) as payment from db_rentalpayments where rental_id='$sales_id'");
		$sum_of_payments = $q8->row()->payment;


		$payble_total = $this->db->query("select coalesce(sum(grand_total),0) as total from db_rented where id='$sales_id'")->row()->total;
		...
		$q7 = $this->db->query("update db_rented set 
		...
		$q12 = $this->db->query("update db_customers set sales_due=(select COALESCE(SUM(grand_total),0)-COALESCE(SUM(paid_amount),0) from db_rented where customer_id='$customer_id' and rent_status='Final') where id=$customer_id");
		if (!$q7) {
			return false;
		} else {
			return true;
	...
	function update_rental_payment_status($sales_id = null, $customer_id = null)
	{
		...
		if (empty($sales_id)) { //If sales ID not exist you need setup all the customers sales due
			...
			if ($q11->num_rows() > 0) {
				foreach ($q11->result() as $res) {
					...
					$q12 = $this->db->query("select id from db_rented where customer_id=" . $res->id);
					if ($q12->num_rows() > 0) {
						foreach ($q12->result() as $res12) {
							if (!$this->update_rental_payment_status_by_sales_id($res12->id, $res->id)) {
								return false;
							}
						}
					} else {
			...
			if (!$this->update_rental_payment_status_by_sales_id($sales_id, $customer_id)) {
				return false;
			}
			return true;
		}
	...
	public function get_items_info($rowcount, $item_id)
	{
		...
		$res1 = $this->db->select('*')->from('db_rentalitems')->where("id=$item_id")->get()->row();
		$q3 = $this->db->query("select * from db_tax where id=" . $res1->tax_id)->row();

		// $batches = $this->db->select('*')->from('db_batches')->where("pro_id=$item_id")->get();

		//Get Customer Price 
		$rental_price = get_price_level_price($customer_id, $res1->r_price);
		$rental_price = number_format($rental_price, decimals(), '.', '');
		...
		$item_tax_amt = ($res1->tax_type == 'Inclusive') ? calculate_inclusive($rental_price, $q3->tax) : calculate_exclusive($rental_price, $q3->tax);
		...
		$info = array(
			...
			'item_price' 				=> $res1->r_price,
			'item_rental_price' 			=> $rental_price,
			'item_tax_name' 			=> $q3->tax_name,
			'item_sales_qty' 			=> ($item_available_qty < 1) ? $item_available_qty : number_format(1, 2),
			'item_tax_id' 				=> $q3->id,
			'item_tax' 					=> $q3->tax,
	...
	public function return_row_with_data($rowcount, $info)
	{
		...
		extract($info);
		$item_amount = ($item_rental_price * $item_sales_qty) + $item_tax_amt;
?>
		<tr id="row_<?= $rowcount; ?>" data-row='<?= $rowcount; ?>'>
			...
			<!-- for size and color -->
			<td id="td_<?= $rowcount; ?>_888">
				<input readonly type="text" name="td_data_<?= $rowcount; ?>_888" id="td_data_<?= $rowcount; ?>_888" class="form-control text-right no-padding only_currency text-center" value="<?= $color; ?>">
			</td>

			<td id="td_<?= $rowcount; ?>_999">
				<input readonly type="text" name="td_data_<?= $rowcount; ?>_999" id="td_data_<?= $rowcount; ?>_999" class="form-control text-right no-padding only_currency text-center" value="<?= $size; ?>">
			</td>
			<!-- for size and color close -->

			<!-- Unit Cost Without Tax-->
			<td id="td_<?= $rowcount; ?>_10">
				<input type="text" name="td_data_<?= $rowcount; ?>_10" id="td_data_<?= $rowcount; ?>_10" class="form-control text-right no-padding only_currency text-center" onkeyup="calculate_tax(<?= $rowcount; ?>)" value="<?= store_number_format($item_rental_price, 0); ?>">
			</td>
			...
			<input type="hidden" id="td_data_<?= $rowcount; ?>_4" name="td_data_<?= $rowcount; ?>_4" value="<?= $item_rental_price; ?>">
	...
	{
		$q1 = $this->db->query("select * from db_rented where id=$sales_id");
		$res1 = $q1->row();
		...
		$sales_date = $res1->rental_date;
		$reference_no = $res1->reference_no;
		$sales_code = $res1->rental_code;
		$sales_note = $res1->rental_note;
	...
	public function save_payment()
	{
		...
		if ($amount > 0 && !empty($payment_type)) {
			$this->db->query("ALTER TABLE db_rentalpayments AUTO_INCREMENT = 1");

			$this->db->trans_begin();

			$payment_code = get_init_code('rental_payment');
			$salespayments_entry = array(
				'store_id' 			=> get_rental_details($sales_id)->store_id,
				'payment_code' 		=> $payment_code,
				'count_id'	  		=> get_count_id('db_rentalpayments'),
				'rental_id' 			=> $sales_id,
								...
								<div class="row">
									<div class="col-md-12">
										...
										<table class="table table-bordered">
											...
											<tbody>
												<?php
												$q1 = $this->db->query("select * from db_rentalpayments where rental_id=$sales_id");
												$i = 1;
												$str = '';
												if ($q1->num_rows() > 0) {
													foreach ($q1->result() as $res1) {
