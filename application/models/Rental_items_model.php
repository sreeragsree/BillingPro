...
class Rental_items_model extends CI_Model
{

	//Datatable start
	var $table = 'db_rentalitems as a';
	...
	public function save_record($modal_post = array())
	{
		...
		validate_package_offers('max_items', 'db_rentalitems');
		//END
		...
		$this->db->query("ALTER TABLE db_rentalitems AUTO_INCREMENT = 1");
		//end
		...
		if ($item_group == 'Single') {
			...
			if ($command == 'save') {
				$initial = array(
					'store_id' 					=> $store_id,
					'count_id' 					=> get_count_id('db_rentalitems'),
			...
			if ($command == 'save') {
				...
				$q3 = $this->db->select("id")->where('item_name', $item_name)->get("db_rentalitems");
				if ($q3->num_rows() > 0) {
					// $this->session->set_flashdata('error', 'Item name already exist...');
					return "failed";
				} else {
					$query1 = $this->db->insert('db_rentalitems', array_merge($info, $initial));
				...
				$item_id = $q_id;
				$query1 = $this->db->where('id', $q_id)->update('db_rentalitems', array_merge($info, $initial));
	...
	{
		//Validate This items already exist or not
		$query = $this->db->query("select * from db_rentalitems where id='$id' ");
		if ($query->num_rows() == 0) {
			show_404();
			exit;
		} else {
			...
			$data['sales_price'] = store_number_format($query->r_price, 0);
			...
			$data['alph_price'] = $query->alphabet_price;

			$data['wholesale_price'] = $query->wholesale_price;
			$data['rac_no'] = $query->rac_no;
	...
	public function delete_items_from_table($ids)
	{
		...
		$purchase_ret_rec = $this->db->select("*")
		...
		if ($purchase_ret_rec->num_rows() > 0) {
			$i = 1;
			echo "Can't Delete!<br>These Items List Have the Rental Records!";
		...
		$q1 = $this->db->query("delete from db_rentalitems where id in($ids)");
...
?>
		<tr id="row_<?= $rowcount; ?>" data-row='<?= $rowcount; ?>'>
			...
			<td id="td_<?= $rowcount; ?>_1">
				<!-- item name  -->
				<select type="text" style="font-weight: bold;" id="tr_batch_id_<?= $rowcount; ?>_10" name="tr_batch_id_<?= $rowcount; ?>_10" class="form-control no-padding">

					<option value="">Choose</option>
					<?php
					foreach ($info['batch'] as $res) {
					?> <option value="<?= $res['id']; ?>"> <?= $res['batch_no']; ?> </option> <?php
																							}
																								?>
		...
		<div style="width:3in; display:flex; flex-wrap:wrap; gap:0px;">
			...
			?>
						<div style="width:1.5in; height:0.98in; display:flex; flex-direction:column; align-items:center; justify-content:center; 
							...
							<img style="max-height:0.35in; width:100%;" src="<?php echo base_url(); ?>barcode/index/<?php echo urldecode($bar_code); ?>">
						</div>
			<?php
					}
				}
