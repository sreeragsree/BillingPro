...
class Sales_model extends CI_Model
{
	...
	private function _get_datatables_query()
	{
		...
		$this->db->select($this->column_order, " b.customer_name, c.coupon_code, GROUP_CONCAT(sp.payment_type ORDER BY sp.payment_type SEPARATOR ',') AS payment_types", FALSE);
		$this->db->from($this->table);
		$this->db->join('db_customers as b', 'b.id=a.customer_id', 'left');
		$this->db->join('db_customer_coupons as c', 'c.id=a.coupon_id', 'left');
		$this->db->join('db_salespayments as sp', 'sp.sales_id = a.id', 'left');
	...
	public function get_sales_with_payment_types()
	{
		$privileged_warehouses = get_privileged_warehouses_ids();
		$warehouse_id = $this->input->post('warehouse_id');
		$customer_id = $this->input->post('customer_id');
		$sales_from_date = system_fromatted_date($this->input->post('sales_from_date'));
		$sales_to_date = system_fromatted_date($this->input->post('sales_to_date'));
		$users = $this->input->post('users');
		$store_id = get_current_store_id();
		$inv_username = strtoupper($this->session->userdata('inv_username'));
        ...
        SELECT 
            ...
            b.customer_name, 
            c.coupon_code, 
            GROUP_CONCAT(sp.payment_type ORDER BY sp.payment_type SEPARATOR ',') AS payment_types 
        FROM db_sales a 
        LEFT JOIN db_customers b ON b.id = a.customer_id 
        LEFT JOIN db_customer_coupons c ON c.id = a.coupon_id 
        LEFT JOIN db_salespayments sp ON sp.sales_id = a.id 
        WHERE 1=1";

		// Dynamic filters
		if (!empty($warehouse_id)) {
			$sql .= " AND a.warehouse_id = " . (int)$warehouse_id;
		} else if (!is_admin() && !is_store_admin()) {
			$sql .= " AND a.warehouse_id IN ($privileged_warehouses)";
		}

		if (!empty($customer_id)) {
			$sql .= " AND a.customer_id = " . (int)$customer_id;
		}

		$sql .= " AND a.store_id = " . (int)$store_id;

		if (!is_admin() && $this->session->userdata('role_id') != '2' && !permissions('show_all_users_sales_invoices')) {
			$sql .= " AND UPPER(a.created_by) = " . $this->db->escape($inv_username);
		}

		if (!empty($users)) {
			$sql .= " AND UPPER(a.created_by) = " . $this->db->escape(strtoupper($users));
		}

		if ($sales_from_date !== '1970-01-01') {
			$sql .= " AND a.sales_date >= " . $this->db->escape($sales_from_date);
		}

		if ($sales_to_date !== '1970-01-01') {
			$sql .= " AND a.sales_date <= " . $this->db->escape($sales_to_date);
		}

		// Grouping
		$sql .= " GROUP BY a.id";

		// Ordering (optional, for datatables)
...
//        return item batches with quantity is > 0
        $batches = $this->db->select('*')
            ->from('db_batches')
            ->where("pro_id", $item_id)
            ->where("quantity >", 0) // only batches with quantity > 0
            ->get();
		...
		<tr id="row_<?= $rowcount; ?>" data-row='<?= $rowcount; ?>'>
			...
			<td id="td_<?= $rowcount; ?>_111">
				...
				<select required onchange="batch_change(this,<?= $rowcount; ?>,<?= $item_id; ?>)" type="text" style="font-weight: bold;" id="tr_batch_id_<?= $rowcount; ?>_111" name="tr_batch_id_<?= $rowcount; ?>_111" class="form-control no-padding batchListing">
					...
					<?php
					if (count($info['batch']) > 0) {
                        foreach ($info['batch'] as $key => $res) {
					?>

                            <option
                                    value="<?= $res['id']; ?>"
                                <?= ($batch_id != 0 && $res['id'] == $batch_id) || ($batch_id == 0 && $key == 0) ? 'selected' : ''; ?>>
                                <?= $res['sales_price']; ?> - <?= $res['alphabet_price']; ?>
                            </option>
					<?php
						}
					}
