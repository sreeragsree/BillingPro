...
class Purchase_model extends CI_Model
{
	...
	//save and update purchase to db 
	public function verify_save_and_update()
	{
		...
		for ($i = 1; $i <= $rowcount; $i++) {
			...
			if (isset($_REQUEST['tr_item_id_' . $i]) && !empty($_REQUEST['tr_item_id_' . $i])) {
				...
				$sales_price 		= $this->xss_html_filter(trim($_REQUEST['td_data_' . $i . '_41']));
				$wholesale_price 	= $this->xss_html_filter(trim($_REQUEST['td_data_' . $i . '_42']));
				$alphabet_price 	= $this->xss_html_filter(trim($_REQUEST['td_data_' . $i . '_43']));
				$mrp_price 			= $this->xss_html_filter(trim($_REQUEST['td_data_' . $i . '_44']));

				$batch_id 			= $this->xss_html_filter(trim($_REQUEST['batch_id_' . $i]));
				...
				// save item in to item table 

				// 1. check if the item with same batch is present in item table run update :
				if ($this->check_item_with_batch_exist_in_items($item_id, $batch_id)) {
					// item exists
					...
					$data = array(
						...
						'sales_price' => $sales_price,
						'mrp' => $mrp_price,
						'alph_price' => $alphabet_price,
						'wholesale_price' => $wholesale_price
					);

					$this->db->where('id', $item_id);
					$this->db->where('batch_id', $batch_id);
					$this->db->update('db_items', $data); // Update the table


				}

				//  check for batch table :
				if (!$this->check_item_with_batch_in_batch_table($item_id, $batch_id)) {
					...
					$batch_entry = array(
						...
						'wholesale_price' 		=> $wholesale_price,
						'mrp_price' 	=> $mrp_price,
						'alphabet_price' 	=> $alphabet_price,
						'quantity' 	=> $purchase_qty,
					);

					$this->db->insert('db_batches', $batch_entry);

					// $this->db->where('pro_id', $item_id);
					// $this->db->where('id', $batch_id);
					// $resultaa = $query = $this->db->get('db_batches')->row();

					// $updated_qty = $resultaa->quantity + $purchase_qty;
					...
					// $this->db->where('id', $batch_id);
					// $this->db->where('pro_id', $item_id);
					// $this->db->update('db_batches', $batch_entry); // Update the table


				} else {
					// item not exists	
					// $existing_item = $this->get_item_from_items_with_id($item_id);

					// unset($existing_item['id']);

					//  // Update only the required 5 columns
					//  $existing_item['purchase_price'] = $price_per_unit;
					//  $existing_item['sales_price'] = $sales_price;
					//  $existing_item['mrp'] = $mrp_price;
					//  $existing_item['alph_price'] = $alphabet_price;
					//  $existing_item['wholesale_price'] = $wholesale_price;
					//  $existing_item['batch_id'] = $batch_id;

					//  $this->db->insert('db_items', $existing_item);
				...
				// save item in to item table close

				//UPDATE itemS QUANTITY IN itemS TABLE
				$this->load->model('pos_model');
				$q6 = $this->pos_model->update_items_quantity($item_id);
				$q66 = $this->pos_model->update_stock_in_batch($item_id,$batch_id);
				if (!$q6 && !$q66) {
					return "failed";
				}
			}
		...
		// adding purchae payment to table :
		if ($amount == '' || $amount == 0) {
			$amount = null;
		}
	...
	public function check_item_with_batch_in_batch_table($item_id, $batch_id)
	{
		$this->db->where('pro_id', $item_id);
		$this->db->where('id', $batch_id);
		$query = $this->db->get('db_batches'); // Change 'items' to your actual table name

		return ($query->num_rows() > 0); // Returns TRUE if item exists, FALSE otherwise
	}

	public function check_item_with_batch_exist_in_items($item_id, $batch_id)
	{
		$this->db->where('id', $item_id);
		$this->db->where('batch_id', $batch_id);
		$query = $this->db->get('db_items'); // Change 'items' to your actual table name

		return ($query->num_rows() > 0); // Returns TRUE if item exists, FALSE otherwise
	}

	public function get_item_from_items_with_id($item_id)
	{
		$this->db->where('id', $item_id);
	...
	// in purchase view item search and add to grid 
	public function get_items_info($rowcount, $item_id)
	{
		...
		$q4 = $this->db->query("select * from db_batches where pro_id=" . $item_id)->result();
		...
		<tr id="row_<?= $rowcount; ?>" data-row='<?= $rowcount; ?>'>
			...
			<td>
				<div class="input-group">
					<select onchange="batch_change(this,<?= $rowcount; ?>,<?= $item_id; ?>)" class="form-control batchListing" id="batch_id_<?= $rowcount; ?>" name="batch_id_<?= $rowcount; ?>" style="width: 100%;">

						<?php
						$q1 = $this->db->query("select * from db_batches where pro_id=" . $item_id);
						if ($q1->num_rows() > 0) {
						?><option value="">Choose</option><?php
															foreach ($q1->result() as $res1) {
																echo "<option value='" . $res1->id . "'>" . $res1->batch_no . "</option>";
															}
														} else {
					...
					<span class="input-group-addon pointer" onclick="show_batch_modal(<?= $rowcount; ?>)" data-itemid="<?= $item_id; ?>" title="New Batch?"><i class="fa fa-user-plus text-primary fa-lg"></i></span>
				</div>
				<span id="batch_id_msg" style="display:none" class="text-danger"></span>
			</td>
			<!-- added by manoj close -->
			<!-- Qty -->
			...
			<td id="td_<?= $rowcount; ?>_4">
				<input onchange="purchase_price_change(<?= $rowcount; ?>)" type="text" name="td_data_<?= $rowcount; ?>_4" id="td_data_<?= $rowcount; ?>_4" class="form-control text-right no-padding only_currency text-center" onkeyup="calculate_tax(<?= $rowcount; ?>)" value="<?= store_number_format(0, 0); ?>">
			</td>

			<!-- sale price  -->
			<td>
				<input onchange="sale_price_change(<?= $rowcount; ?>)" type="text" name="td_data_<?= $rowcount; ?>_41" id="td_data_<?= $rowcount; ?>_41" class="form-control text-right no-padding only_currency text-center" onkeyup="calculate_tax(<?= $rowcount; ?>)" value="<?= store_number_format(0, 0); ?>">
			</td>

			<!-- wholesale -->
			<td>
				<input onchange="wholesale_price_change(<?= $rowcount; ?>)" type="text" name="td_data_<?= $rowcount; ?>_42" id="td_data_<?= $rowcount; ?>_42" class="form-control text-right no-padding only_currency text-center" onkeyup="calculate_tax(<?= $rowcount; ?>)" value="<?= store_number_format(0, 0); ?>">
			</td>

			<!--  alphabet   -->
			<td>
				<input type="text" name="td_data_<?= $rowcount; ?>_43" id="td_data_<?= $rowcount; ?>_43" class="form-control text-right no-padding only_currency text-center" onkeyup="calculate_tax(<?= $rowcount; ?>)" value="<?= store_number_format(0, 0); ?>">
			</td>

			<!-- mrp  -->
			<td>
				<input onchange="mrp_prices(<?= $rowcount; ?>)" type="text" name="td_data_<?= $rowcount; ?>_44" id="td_data_<?= $rowcount; ?>_44" class="form-control text-right no-padding only_currency text-center" onkeyup="calculate_tax(<?= $rowcount; ?>)" value="<?= store_number_format(0, 0); ?>">
			</td>
			...
			<!-- Tax Amount -->
			<td id="td_<?= $rowcount; ?>_5"><input type="text" name="td_data_<?= $rowcount; ?>_5" id="td_data_<?= $rowcount; ?>_5" class="form-control text-right no-padding only_currency text-center" readonly value="<?= store_number_format(0, 0); ?>"></td>

			<!-- Unit Cost -->
			<td id="td_<?= $rowcount; ?>_10"><input type="text" name="td_data_<?= $rowcount; ?>_10" id="td_data_<?= $rowcount; ?>_10" class="form-control text-right no-padding only_currency text-center" readonly value="<?= store_number_format(0, 0); ?>"></td>

			<!-- Amount -->
			<td id="td_<?= $rowcount; ?>_9"><input type="text" name="td_data_<?= $rowcount; ?>_9" id="td_data_<?= $rowcount; ?>_9" class="form-control text-right no-padding only_currency text-center" style='border-color: #f39c12;' title='Total' readonly value="<?= store_number_format(0, 0); ?>"></td>
