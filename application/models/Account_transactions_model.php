...
class Account_transactions_model extends CI_Model
{
	...
	{
		$this->db->select('SUM(a.debit_amt) as total_debit, SUM(a.credit_amt) as total_credit');
		$this->db->from($this->table);
		$this->db->where("a.store_id", get_current_store_id());
		...
		if (isset($_POST['account_id'])) {
			$account_id = $_POST['account_id'];
			$this->db->group_start();
			$this->db->where('a.credit_account_id', $account_id);
			$this->db->or_where('a.debit_account_id', $account_id);
			$this->db->group_end();
		}

		$from_date = $this->input->post('from_date');
		$from_date = system_fromatted_date($from_date);
		if (!empty($from_date) && $from_date != '1970-01-01') {
			$this->db->where("a.transaction_date >=", $from_date);
		}

		$to_date = $this->input->post('to_date');
		$to_date = system_fromatted_date($to_date);
		if (!empty($to_date) && $to_date != '1970-01-01') {
			$this->db->where("a.transaction_date <=", $to_date);
		}
		...
		$query = $this->db->get();
		return $query->row(); // returns an object with total_debit and total_credit
	}
