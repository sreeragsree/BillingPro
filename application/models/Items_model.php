...
class Items_model extends CI_Model
{
    ...
    public function get_item_details($item_id) {
      $this->db->select('i.*, c.category_name, u.unit_name, b.brand_name, 
                  ...
                  t.tax_name, t.tax as tax_percentage,
                  i.supplier_item_code');
        $this->db->from('db_items i');
        $this->db->join('db_categories c', 'c.id = i.category_id', 'left');
        ...
        $this->db->where('i.store_id', get_current_store_id());
    ...
    public function save_record($modal_post = array())
    {
        ...
        if ($item_group == 'Single') {
            ...
            if ($command == 'save') {
                ...
                // check item name is already present or not
                $q3 = $this->db->select("id")->where('item_name', $item_name)->get("db_items");
                if ($q3->num_rows() > 0) {
                    // $this->session->set_flashdata('error', 'Item name already exist...');
                    return "failed";
                } else {
                    $query1 = $this->db->insert('db_items', array_merge($info, $initial));
                    if (!$query1) {
                        log_message('error', 'Insert failed: ' . json_encode($this->db->error()));
                        return "failed to insert item" . $this->db->error();
                    }
                    $item_id = $this->db->insert_id();

                    // update batch table too
                   $adjustment_qty = !empty($adjustment_qty) ? $adjustment_qty : 0;
                   ...
                   if ($adjustment_qty >= 0) {
                    ...
                    $this->db->insert('db_batches', $batch_array);
                    $batch_id = $this->db->insert_id();

                    $updated_item_list = array(
                        'batch_id' => $batch_id
                    );

                    $this->db->where('id', $item_id)->update('db_items', $updated_item_list);
                    // update batch table too close
                }
                // check item name is already present or not close
                ...
                // check is to add on batch table

            }

            if (!$query1) {
                return "failed to update batch in item table";
            }
            ...
            if ($adjustment_qty > 0) {
                ...
                if (!$q2) {
                    return "failed to add opening stock";
                }
            }
        } //Single END
    ...
    public function get_details($id, $data)
    {
        ...
        $query_batch = $this->db->query("select * from db_batches where upper(pro_id)=upper('$id')");
        ...
        if ($query->num_rows() == 0) {
            ...
            $data['alph_price'] = $query->alph_price;
            $data['seller_code'] = $query->seller_code;
            $data['seller_name'] = $query->seller_name;

            $data['batches'] = $query_batch->result_array();
...
//	batch edit modal
    public function editBatchRecordModal()
    {
        ...
        $edit_batid = $this->input->post('edit_batid');

        $edit_batch = $this->input->post('edit_batch');
        $edit_pprice = $this->input->post('edit_pprice');
        $edit_sprice = $this->input->post('edit_sprice');
        $edit_wprice = $this->input->post('edit_wprice');
        $edit_mrp = $this->input->post('edit_mrp');
        $edit_alpprice = $this->input->post('edit_alpprice');

        $ids = [$id];

        // Check if ONLY sales_price is being updated
        $batch_data = $this->db->where('id', $edit_batid)->get('db_batches')->row();

        $updating_only_sales_price = (
            $batch_data &&
            $batch_data->batch_no == $edit_batch &&
            $batch_data->purchase_price == $edit_pprice &&
            $batch_data->wholesale_price == $edit_wprice &&
            $batch_data->mrp_price == $edit_mrp &&
            $batch_data->alphabet_price == $edit_alpprice &&
            $batch_data->sales_price != $edit_sprice // only this one is different
        );

        // Only block full update if not updating just the sales price
        if (!$updating_only_sales_price) {

            $tables_to_check = [
                'db_purchaseitemsreturn' => 'Purchase Returns',
                'db_purchaseitems' => 'Purchases',
                'db_salesitemsreturn' => 'Sales Returns',
                'db_salesitems' => 'Sales',
                'db_quotationitems' => 'Quotations',
            ];
            ...
            foreach ($tables_to_check as $table => $label) {
                ...
                if ($query->num_rows() > 0) {
                    $i = 1;
                    echo "Can't Update!<br>These Items Have {$label} Records!";
        ...
        $batch_array = array(
            'batch_no' => $edit_batch,
            'purchase_price' => $edit_pprice,
            'sales_price' => $edit_sprice,
            'wholesale_price' => $edit_wprice,
            'mrp_price' => $edit_mrp,
            'alphabet_price' => $edit_alpprice,
        );

        $result = $this->db->where('id', $edit_batid)->update('db_batches', $batch_array);
...
//	batch edit modal close
    ...
    public function get_purchase_items_info($rowcount, $item_id, $purchase_qty, $purchase_batch_id = '')
    {
        // Get item info
        ...
        $all_batches = $this->db->select('id, batch_no, sales_price')
            ->from('db_batches')
        ...
        $info['batch'] = $all_batches;
        $info['purchase_batch_id'] = $purchase_batch_id; // Store for preselect
    ...
    {
        $q1 = $this->db->select('*')->from('db_items')->where("id=$item_id")->get();
        $q2forbatch = $this->db->select('*')->from('db_batches')->where("pro_id=$item_id")->get();
        $tax = $this->db->query("select tax from db_tax where id=" . $q1->row()->tax_id)->row()->tax;
        ...
        $info['wholesale_price'] = $q1->row()->wholesale_price;
        $info['batch'] = $q2forbatch->result_array();
    ...
    public function return_row_with_data($rowcount, $info)
    {
        ...
        <tr id="row_<?= $rowcount; ?>" data-row='<?= $rowcount; ?>'>
            <td id="td_<?= $rowcount; ?>_1">
                <input type="text" style="font-weight: bold;" id="td_data_<?= $rowcount; ?>_1"
                       class="form-control no-padding" value='<?= $item_name; ?>' readonly>
            </td>

            <!-- Batch selector -->
            <td id="td_<?= $rowcount; ?>_2">
                <select style="font-weight: bold;"
                        id="tr_batch_id_<?= $rowcount; ?>_10"
                        name="tr_batch_id_<?= $rowcount; ?>_10"
                        class="form-control no-padding batchListing">
                    <option value="">Choose</option>
                    <?php foreach ($batch as $res): ?>
                        <option value="<?= $res['id']; ?>"
                            <?= ($res['id'] == $purchase_batch_id) ? 'selected' : ''; ?>>
                            <?= $res['batch_no']; ?> - <?= $res['sales_price']; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </td>
            ...
            <td id="td_<?= $rowcount; ?>_3">
                ...
                <span class="input-group-btn">
                    <button onclick="decrement_qty(<?= $rowcount; ?>)" type="button"
                            class="btn btn-default btn-flat">
                        <i class="fa fa-minus text-danger"></i>
                    </button>
                </span>
                    <input type="text" value="<?= $item_sales_qty; ?>"
                           class="form-control no-padding text-center"
                           onkeyup="calculate_tax(<?= $rowcount; ?>)"
                           id="td_data_<?= $rowcount; ?>_3"
                           name="td_data_<?= $rowcount; ?>_3">
                    <span class="input-group-btn">
                    <button onclick="increment_qty(<?= $rowcount; ?>)" type="button"
                            class="btn btn-default btn-flat">
            ...
            <td id="td_<?= $rowcount; ?>_16" style="text-align: center;">
                <a class="fa fa-fw fa-minus-square text-red" style="cursor: pointer;font-size: 34px;"
                   onclick="removerow(<?= $rowcount; ?>)" title="Delete ?"
                   name="td_data_<?= $rowcount; ?>_16"
                   id="td_data_<?= $rowcount; ?>_16"></a>
            </td>

            <input type="hidden" id="tr_available_qty_<?= $rowcount; ?>_13"
        ...
        <div style="width:3in; display:flex; flex-wrap:wrap; gap:0px;">
            ...
            for ($i = 1; $i <= $rowcount; $i++) {
                if (isset($_POST['tr_item_id_' . $i]) && !empty($_POST['tr_item_id_' . $i])) {
                    ...
                    $alph_price = $batchDetails->alphabet_price;
                    $bar_code = $res1->custom_barcode."-".$batchDetails->id;

                    for ($j = 1; $j <= $item_count; $j++) {
                        ?>
                        <div style="width:1.5in; height:0.98in; display:flex; flex-direction:column; align-items:center; justify-content:center;
									text-align:center; ">
                            <b class="text-uppercase">K D C</b>
                            <span><?php echo $item_name; ?></span>
                            <span><?php echo $alph_price; ?></span>
                            <img style="max-height:0.35in; width:100%;"
                                 src="<?php echo base_url(); ?>barcode/index/<?php echo urldecode($bar_code); ?>">
                        </div>
                        <?php
                    }
                }
    ...
    public function return_variant_data_in_html_row($rowcount, $info)
    {
        ...
        <tr id="row_<?= $rowcount; ?>" data-row='<?= $rowcount; ?>'>
            ...
            <!-- SKU-->
            <td id="td_<?= $rowcount; ?>_2"><input type="text" name="td_data_<?= $rowcount; ?>_2"
                                                   id="td_data_<?= $rowcount; ?>_2"
                                                   class="form-control text-center no-padding"
                                                   value="<?= $variant_item_sku; ?>" placeholder='Optional'></td>

            <!-- HSN-->
            <td id="td_<?= $rowcount; ?>_9"><input type="text" name="td_data_<?= $rowcount; ?>_9"
                                                   id="td_data_<?= $rowcount; ?>_9"
                                                   class="form-control text-center no-padding"
                                                   value="<?= $variant_item_hsn; ?>" placeholder='Optional'></td>

            <!-- Barcode-->
            <td id="td_<?= $rowcount; ?>_8"><input type="text" name="td_data_<?= $rowcount; ?>_8"
                                                   id="td_data_<?= $rowcount; ?>_8"
                                                   class="form-control text-center no-padding" value="<?= $barcode; ?>"
                                                   placeholder='Optional'></td>

            <!-- Price-->
            <td id="td_<?= $rowcount; ?>_3"><input type="text" name="td_data_<?= $rowcount; ?>_3"
                                                   id="td_data_<?= $rowcount; ?>_3"
                                                   class="form-control text-right no-padding only_currency text-center"
                                                   onkeyup='calculate_purchase_price_of_all_row()'
                                                   placeholder='Required' style="border-color: #f39c12;"
                                                   value="<?= $variant_purchase_price; ?>"></td>

            <!-- Purchase Price-->
            <td id="td_<?= $rowcount; ?>_4"><input type="text" name="td_data_<?= $rowcount; ?>_4"
                                                   id="td_data_<?= $rowcount; ?>_4"
                                                   class="form-control text-right no-padding only_currency text-center"
                                                   placeholder='Required' style="border-color: #f39c12;" value=""
                                                   readonly></td>

            <!-- Profit Margin-->
            <td id="td_<?= $rowcount; ?>_5"><input type="text" name="td_data_<?= $rowcount; ?>_5"
                                                   id="td_data_<?= $rowcount; ?>_5"
                                                   class="form-control text-right no-padding only_currency text-center"
                                                   placeholder='Required' onchange='calculate_sales_price_of_all_row()'
                                                   style="border-color: #f39c12;"
                                                   value="<?= $variant_profit_margin; ?>"></td>

            <!-- Sales Price -->
            <td id="td_<?= $rowcount; ?>_6"><input type="text" name="td_data_<?= $rowcount; ?>_6"
                                                   id="td_data_<?= $rowcount; ?>_6"
                                                   class="form-control text-right no-padding only_currency text-center"
                                                   placeholder='Required'
                                                   onchange='calculate_profit_margin_of_all_row()'
                                                   style="border-color: #f39c12;" value="<?= $item_sales_price; ?>">
            </td>
            ...
            <td id="td_<?= $rowcount; ?>_10"><input type="text" name="td_data_<?= $rowcount; ?>_10"
                                                    ...
                                                    class="form-control text-right no-padding only_currency text-center"
                                                    placeholder='Optional' value="<?= $item_mrp; ?>"></td>

            <!-- Opening Stock -->
            <td id="td_<?= $rowcount; ?>_11"><input type="text" name="td_data_<?= $rowcount; ?>_11"
                                                    id="td_data_<?= $rowcount; ?>_11"
                                                    class="form-control text-right no-padding only_currency text-center"
                                                    placeholder='Optional' value="<?= $opening_stock; ?>"></td>


            <!-- Delete button -->
            <td id="td_<?= $rowcount; ?>_7" style="text-align: center;">
                <a class=" fa fa-fw fa-minus-square text-red" style="cursor: pointer;font-size: 34px;"
                   onclick="removerow_also_delete_from_database('<?= $item_id; ?>',<?= $rowcount; ?>)" title="Delete ?"
                   name="td_data_<?= $rowcount; ?>_7" id="td_data_<?= $rowcount; ?>_7"></a>
            </td>
            <input type="hidden" id="tr_variant_id_<?= $rowcount; ?>" name="tr_variant_id_<?= $rowcount; ?>"
                   value="<?= $variant_id; ?>">
            <input type="hidden" id="count_id_<?= $rowcount; ?>" name="count_id_<?= $rowcount; ?>"
                   value="<?= $count_id; ?>">
            <input type="hidden" id="item_code_<?= $rowcount; ?>" name="item_code_<?= $rowcount; ?>"
                   value="<?= $item_code; ?>">
        </tr>
        <?php
    ...
    public function add_opening_stock(array $info)
    {
        ...
        try {
            ...
            if ($insert) {
                ...
                $query222 = $this->pos_model->update_stock_in_batch($info['item_id'], $info['batch_id']);
                if ($query2 && $query222) {
