...
class Account_transactions extends MY_Controller
{
	...
	public function get_rental_code($salespayment_id)
	{
		if (empty($salespayment_id)) {
			return '';
		}
		return $this->db->select("a.rental_code,a.id")
			->from("db_rented a")
			->from("db_rentalpayments b")
			->where("a.id=b.rental_id")
	...
	public function ajax_list()
	{
		...
		$to_date = system_fromatted_date($this->input->post('to_date'));
...
//			    add row for rental
                if (!empty($accounts->customer_id)) {
                    if (!empty($accounts->ref_rentalpayments_id)) {
                        $query = $this->get_rental_code($accounts->ref_rentalpayments_id);
                        $sales_code = $query->rental_code;
                        $sales_id = $query->id;
                        $from_ = "<a data-toggle='tooltip' title='View Rental Payments' href='" . base_url('rental/invoice/') . $sales_id . "'>" . $sales_code . "</a>";
                    }
                }
//			    add row for rental close
			...
			$sum_debit += ($account_cr_dr == 'Debit_entry') ? store_number_format($accounts->debit_amt) : 0;
			$sum_credit += ($account_cr_dr == 'Credit_entry') ? store_number_format($accounts->credit_amt) : 0;
		...
		$totals = $this->accounts->get_totals();
