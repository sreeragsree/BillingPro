...
class Rental extends MY_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->load_global();
		$this->load->model('rented_model', 'sales');
	...
	public function index()
	{
		...
		$data = $this->data;
		$data['page_title'] = 'Rental List';
		$this->load->view('rented-list', $data);
	}
	...
	public function add_rental()
	{
		...
		$data = $this->data;
		$data['page_title'] = $this->lang->line('rental');
	...
	public function ajax_list()
	{
		...
		foreach ($list as $sales) {
			...
			$row[] = show_date($sales->rental_date);

			#-----------------------------------------------------
			$date_difference = date_difference($sales->return_date, date("Y-m-d"));
			$str = '';
			//$info='';
			if ($date_difference > 0) {
			    if($sales->rental_status != 'Returned')
                {
			...
			$row[] = (!empty($sales->return_date) ) ? show_date($sales->return_date) . $str : '';
			#-----------------------------------------------------
			...
			$row[] = $sales->rental_code . $info;
			$row[] = get_rental_payment_type($sales->id);
			...
			$str = '';
			if ($sales->rental_status == 'Active')
				$str = "<span class='label label-danger' style='cursor:pointer'>Not Returned </span>";
			if ($sales->rental_status == 'Partial')
				$str = "<span class='label label-warning' style='cursor:pointer'> Partially Returned </span>";
			if ($sales->rental_status == 'Returned' && $sales->returned_flag == 0)
				$str = "<span class='label label-info' style='cursor:pointer'> Returned Inspection Pending </span>";
			if ($sales->rental_status == 'Returned' && $sales->returned_flag == 1)
				$str = "<span class='label label-success' style='cursor:pointer'> Returned Inspection Completed </span>";
			...
			$str2 .= '<li>
												<a title="View Invoice" href="' . base_url() . 'rental/invoice/' . $sales->id . '" >
													<i class="fa fa-fw fa-eye text-blue"></i>View Rental
												</a>
											</li>';
			...
			if ($sales->rental_status != 'Returned')
			$str2 .= '<li>
												<a title="Return Item" href="' . base_url() . 'rental/return_rented_item/' . $sales->id . '">
													<i class="fa fa-history text-blue"></i>Mark as return
												</a>
											</li>
											';
			...
			if ($sales->rental_status == 'Returned' && $sales->returned_flag == 0)
			$str2 .= '<li>
												<a title="Inspect & Update Stock" href="' . base_url() . 'rental/inspect_and_update_stock/' . $sales->id . '">
													<i class="fa fa-fw fa-file-pdf-o text-blue"></i>Inspect & Update Stock
												</a>
											</li>
											';
	...
	{
		$result = $this->sales->inspect_and_update_stock($rent_id);
		// echo $result;
		$this->session->set_flashdata('stock_update_result', $result);
	...
	{
		$result = $this->sales->return_rented_item($rent_id);
		// echo $result;
		$this->session->set_flashdata('return_rented_item', $result);
		// Redirect back to the referring page
		redirect($_SERVER['HTTP_REFERER']);
	}
	...
	public function invoice($id)
	{
		...
		$data = $this->data;
		$data = array_merge($data, array('sales_id' => $id));
		$data['page_title'] = 'Rental Invoice';

		$this->load->view('rental-invoice', $data);
	}
