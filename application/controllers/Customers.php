...
//        print_r($data);
//        echo '</pre>';
        $this->load->view('customer-credit-bybills', $data);
    }

    public function viewcredits_detail()
    {
        // Ensure it's a POST request and customer_id is provided
        ...
        $order_column_index = $this->input->post('order')[0]['column'] ?? 1;
        $order_dir = $this->input->post('order')[0]['dir'] ?? 'desc';

        // Map column index to database columns - adjust as per your table
        $columns = [
            0 => '',             // Checkbox/no ordering
            1 => 'sales_code',
            2 => 'grand_total',
            3 => 'paid_amount',
            4 => '(grand_total - paid_amount)'  // balance due
        ];
        $order_column = $columns[$order_column_index] ?? 'sales_code';

        // Get total records count (active with due > 0)
        ...
        $this->db->select('sales_code, grand_total, paid_amount, (grand_total - paid_amount) as credits_due, id');
        $this->db->from('db_sales');
        $this->db->where('customer_id', $customer_id);
        $this->db->where('status', 1);
        $this->db->where('(grand_total - paid_amount) >', 0);
        ...
        foreach ($query->result() as $row) {
            $checkbox = '<input type="checkbox" class="column_checkbox" data-id="' . $row->id . '">';
            // Prepare row data as array in order of columns
            $data[] = [
                $checkbox,
                htmlspecialchars($row->sales_code),
                number_format($row->grand_total, 2),
                number_format($row->paid_amount, 2),
                number_format($row->credits_due, 2)
            ];
        }

        // No filtering on search in this simple example
			...
			$row[] = $str;
					$str2 = '<div class="btn-group" title="View Account">
										...
										<ul role="menu" class="dropdown-menu dropdown-light pull-right">';
                                            ...
                                            $str2.='<li>
												<a title="View Credits" class="pointer" href="' . site_url('customers/viewcredits/' . $customers->id) . '">
													<i class="fa fa-fw fa-money text-blue"></i>View Credits
												</a>
											</li>';
