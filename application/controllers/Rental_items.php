...
class Rental_items extends MY_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->load_global();
		$this->load->model('rental_items_model', 'items');
	}
	...
	public function index()
	{
		...
		// }
		$data = $this->data;
		$data['page_title'] = 'Rental Items List';
		$this->load->view('rental-items-list', $data);
	}
	public function add()
	{
		// $this->permission_check('add_item');
		$data = $this->data;
		$data['page_title'] = 'Rental Items';
		$this->load->view('rental_items', $data);
		// var_dump($data);
	}
	...
	public function newitems()
	{
		...
		if ($this->input->post('item_group') == 'Single') {
			...
			$this->form_validation->set_rules('rental_price', 'Sales Price', 'trim|required');
		} else {
			if ($this->input->post('existing_row_count') == 1) {
				echo "Variants List Not Added, Please Select Variants!!";
				exit();
			}
		}
	...
	{
		$this->belong_to('db_rentalitems', $id);
		...
		$data = $this->data;
		$this->load->model('rental_items_model');
		$result = $this->items->get_details($id, $data);
	...
	public function update_items()
	{
		...
		if ($this->input->post('item_group') == 'Single') {
			$this->form_validation->set_rules('price', 'Item Price', 'trim|required');
			$this->form_validation->set_rules('rental_price', 'Rental Price', 'trim|required');
		} else {
			if ($this->input->post('existing_row_count') == 1) {
				echo "Variants List Not Added, Please Select Variants!!";
				exit();
	...
	public function ajax_list()
	{
		...
		foreach ($list as $items) {
			...
			$row[] = $items->color;
			$row[] = $items->size;
			$row[] = store_number_format($items->r_price);
			...
			// if ($this->permissions('items_edit'))
				$str2 .= '<li>
												<a title="Edit Record ?" href="' . base_url('rental_items/update/' . $items->id) . '">
	...
	public function get_json_items_details()
	{
		...
		if (isset($search_for) && $search_for == 'purchase') {
			...
			$this->db->from("db_rentalitems as a");
		} else if (isset($search_for) && ($search_for == 'labels' || $search_for == 'rental')) {
			$this->db->select("*");
			$this->db->from("db_rentalitems as a");
		} else {
			// $this->db->where('a.service_bit=0');
			$this->db->select("a.price,a.id,a.item_name,a.item_code,COALESCE(a.stock,0) as stock,item_group");
			$this->db->from("db_rentalitems as a");
		...
		if ($sql) {
			foreach ($sql->result() as $res) {
				if ($res->item_group != 'Variants') {
					...
					$json_arr["rental_price"] = $res->r_price;
					$json_arr["wholesale_price"] = $res->wholesale_price;
					$json_arr["size"] = $res->size;
					$json_arr["color"] = $res->color;
		...
		// else {
		// 	$error = $this->db->error();
		// 	echo "Query failed. Error Code: " . $error['code'] . "<br>";
