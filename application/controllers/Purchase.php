...
class Purchase extends MY_Controller
{
	...
	public function get_item_details_with_batch_and_productid() {
		...
		// purchase_price , sales_price , wholesale_price , mrp_price , alphabet_price

        // $this->db->select('id,purchase_price,sales_price,wholesale_price,alph_price,mrp,discount');
        $this->db->select('id,purchase_price,sales_price,wholesale_price,alphabet_price,mrp_price,pro_id,quantity');
        $this->db->where('id', $batch_id);
        $this->db->where('pro_id', $pro_id);
        $query = $this->db->get('db_batches'); // Replace 'your_table' with your actual table name

        // Check if data exists
        if ($query->num_rows() > 0) {
            echo json_encode($query->row()); // Return single row as JSON
        } else {
	...
	// new batch creation using modal by manoj
	public function newbatch()
	{

		$this->load->model('batch_model');
		$result = $this->batch_model->verify_and_save();
		//fetch latest item details
		$res = array();
		$query = $this->db->query("select id,batch_no from db_batches order by id desc limit 1");
		$res['id'] = $query->row()->id;
		$res['batch_no'] = $query->row()->batch_no;
		$res['result'] = $result;
	...
	public function getallbatch()
	{
		...
		$this->load->model('batch_model');
		$result = $this->batch_model->get_all_batches();
		//fetch latest item details
		$res = array();
		$query = $this->db->query("select id,batch_no from db_batches order by id desc limit 1");
		$res['id'] = $query->row()->id;
