...
<body class="hold-transition skin-blue sidebar-mini">
    <div class="wrapper">
        ...
        $total_invoice = $this->db->select("COUNT(*) as total")->from("db_rented")->where("store_id", get_current_store_id())->get()->row()->total;
        ...
        $sal_total = $this->db->select("COALESCE(sum(grand_total),0) AS tot_sal_grand_total")->from("db_rented")->where("rent_status", 'Final')->where("store_id", get_current_store_id())->get()->row()->tot_sal_grand_total;
        ...
        $tot_received_amt = $this->db->select("COALESCE(SUM(paid_amount),0) AS paid_amount")->from("db_rented")->where("store_id", get_current_store_id())->get()->row()->paid_amount;
        ...
        $sales_due_total = $this->db->select("COALESCE(SUM(rental_due),0) AS rental_due")->from("db_customers")->where("store_id", get_current_store_id())->get()->row()->rental_due;
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="col-md-2 pull-right">
                                            ...
                                            <div class="box-tools">
                                                <a class="btn btn-block btn-info" href="<?php echo $base_url; ?>rental/add_rental">
                                                    <i class="fa fa-plus"></i> Add Rental </a>
                                            </div>
    ...
    <script type="text/javascript">
        ...
        function load_datatable() {
            ...
            var table = $('#example2').DataTable({
                ...
                "ajax": {
                    "url": "<?php echo site_url('rental/ajax_list') ?>",
                    "type": "POST",
                    "data": {
   ...
   <script type="text/javascript">
        function print_invoice(id) {
            window.open("<?= base_url(); ?>pos/print_rental_invoice_pos/" + id, "_blank", "scrollbars=1,resizable=1,height=500,width=500");
        }
