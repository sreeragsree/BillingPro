...
<body class="hold-transition skin-blue sidebar-mini">
  <div class="wrapper">
    ...
    <div class="content-wrapper">
      ...
      //   $q3 = $this->db->query("SELECT b.coupon_id,b.coupon_amt, b.due_date,b.quotation_id,b.store_id,a.customer_name,a.mobile,
      //                        a.phone,a.gstin,a.tax_number,a.email,a.shippingaddress_id,a.id,
      ...
      $q3 = $this->db->query("SELECT b.coupon_id,b.coupon_amt,b.return_date,b.store_id,a.customer_name,a.mobile,
                           a.phone,a.gstin,a.tax_number,a.email,a.shippingaddress_id,a.id,
                           a.opening_balance,a.country_id,a.state_id,a.city,
                           a.postcode,a.address,b.rental_date,b.created_time,b.reference_no,
                           b.rental_code,b.rental_status,b.rental_note,
                           coalesce(b.grand_total,0) as grand_total,
                           coalesce(b.subtotal,0) as subtotal,
                           coalesce(b.paid_amount,0) as paid_amount,
                           coalesce(b.other_charges_input,0) as other_charges_input,
                           b.other_charges_tax_id,
                           coalesce(b.other_charges_amt,0) as other_charges_amt,
      ...
      $sales_date = $res3->rental_date;
      $due_date = (!empty($res3->return_date)) ? show_date($res3->return_date) : '';
      $created_time = $res3->created_time;
      $reference_no = $res3->reference_no;
      $sales_code = $res3->rental_code;
      $sales_status = $res3->rental_status;
      $sales_note = $res3->rental_note;
      $invoice_terms = $res3->rental_note;
      ...
      <section class="invoice">
          ...
          <div class="row">
            <div class="col-xs-12">
              <h2 class="page-header">
                <i class="fa fa-globe"></i> Rental Invoice
                ...
                <tbody>
                  ...
                  foreach ($q2->result() as $res2) {
                    ...
                    if ($res2->tax_type == 'Inclusive') {
                      $price_per_unit -= ($res2->tax_amt / $res2->rent_qty);
                    }

                    $tot_price = $price_per_unit * $res2->rent_qty;
                    ...
                    echo "<td>" . format_qty($res2->rent_qty) . "</td>";
              ...
              <div class="row">
                <div class="col-md-12">
                  <div class="form-group">
                    ...
                    <div class="col-sm-8">
                      <label class="control-label" Ā>: <?= $notes; ?></label>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div class="form-group">
                    <table class="table table-hover table-bordered" style="width:100%" id="">
                      ...
                      <tbody>
                        ...
                        <?php
                        if (isset($sales_id)) {
                          $q3 = $this->db->query("select * from db_rentalpayments where rental_id=$sales_id");
                          if ($q3->num_rows() > 0) {
                            $i = 1;
                            $total_paid = 0;
        ...
        <div class="row no-print">
          <div class="col-xs-6">
            <a accesskey="p" href="<?php echo $base_url; ?>pos/print_rental_invoice_pos/<?php echo  $sales_id ?>" target="_blank" class="btn btn-info">
          ...
          <div class="col-xs-6 text-right">
            <a href="<?php echo $base_url; ?>pdf/rental/<?php echo  $sales_id ?>" target="_blank" class="btn btn-primary">
