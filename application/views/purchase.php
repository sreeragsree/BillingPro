...
<body class="hold-transition skin-blue sidebar-mini">
   ...
   <div class="wrapper">
      ...
      <div class="content-wrapper">
         ...
         <?php include "modals/modal_batch.php"; ?>
                  ...
                  <div class="box box-primary ">
                     ...
                     <div class="box-body">
                        ...
                        if (warehouse_module() && warehouse_count() > 1) {
                           ...
                           echo "<input hidden  name='warehouse_id' id='warehouse_id' value='" . get_store_warehouse_id() . "'>";
                        }
                        ?>
                        <!-- Warehouse Code end -->
                                       ...
                                       <div class="table-responsive" style="width: 100%">
                                          <table class="table table-hover table-bordered" style="width:100%" id="purchase_table">
                                             <thead class="custom_thead">
                                                <tr class="bg-primary">
                                                   <th rowspan='2' style="width:10%"><?= $this->lang->line('item_name'); ?></th>
                                                   <!-- created by manoj -->
                                                   <th rowspan='2' style="width:8%">Batch No</th>
                                                   <!-- created by manoj close-->
                                                   <th rowspan='2' style="width:7%;"><?= $this->lang->line('quantity'); ?></th>

                                                   <th rowspan='2' style="width:5%"><?= $this->lang->line('p_price'); ?>(<?= $CURRENCY; ?>)</th>
                                                   <th rowspan='2' style="width:5%"><?= $this->lang->line('s_price'); ?>(<?= $CURRENCY; ?>)</th>

                                                   <th style="width:5%"><?= $this->lang->line('w_price'); ?>(<?= $CURRENCY; ?>) </th>
                                                   <th style="width:5%"> <?= $this->lang->line('alp_price'); ?>(<?= $CURRENCY; ?>) </th>
                                                   <th style="width:5%"><?= $this->lang->line('mrp_price'); ?>(<?= $CURRENCY; ?>)</th>

                                                   <!--  <th rowspan='2' style="width:7.5%"><?= $this->lang->line('tax'); ?> %</th> -->
                                                   <th rowspan='2' style="width:5%"><?= $this->lang->line('discount'); ?>(<?= $CURRENCY; ?>)</th>
                                                   <th rowspan='2' style="width:5%"><?= $this->lang->line('tax_amount'); ?></th>
                                                   <th rowspan='2' style="width:5%"><?= $this->lang->line('unit_cost'); ?></th>
                                                   <th rowspan='2' style="width:8%"><?= $this->lang->line('total_amount'); ?></th>
   ...
   <script>
      ...
      $("#store_id").on("change", function() {
         ...
         $.post(base_url + "purchase/get_suppliers_select_list", {
            store_id: store_id
         }, function(result) {
      ...
      // check feilds validations ( p.price , s.price , w.price , MRP , discount )
      ...
      // check feilds validations close
         ...
         if (checkForDuplicates()) {
            ...
            $.get(base_url + "purchase/get_item_details_with_batch_and_productid", {
               ...
               function(result) {
                  ...
                  console.log('Json data count : ' + Object.keys(jsonData).length);

                  if (Object.keys(jsonData).length <= 1) {

                     $('#td_data_' + row_id + "_4").val(0);
                     $('#td_data_' + row_id + "_41").val(0);
                     $('#td_data_' + row_id + "_42").val(0);
                     $('#td_data_' + row_id + "_43").val(0);
                     $('#td_data_' + row_id + "_44").val(0);
                     ...
                     $('#td_data_' + row_id + "_41").prop('readonly', false);
                     $('#td_data_' + row_id + "_42").prop('readonly', false);
                     $('#td_data_' + row_id + "_43").prop('readonly', false);
                     $('#td_data_' + row_id + "_44").prop('readonly', false);
                     ...
                     if (parseFloat(jsonData.purchase_price) !== 0) {
                        $('#td_data_' + row_id + "_4").val(jsonData.purchase_price);
                        $('#td_data_' + row_id + "_4").prop('readonly', true);
                     }
                     else
                     {
                        $('#td_data_' + row_id + "_4").val('');
                        $('#td_data_' + row_id + "_4").prop('readonly', false);  
                     }

                     if (parseFloat(jsonData.sales_price) !== 0) {
                        $('#td_data_' + row_id + "_41").val(jsonData.sales_price);
                        $('#td_data_' + row_id + "_41").prop('readonly', true);
                     }
                     else
                     {
                        $('#td_data_' + row_id + "_41").val('');
                        $('#td_data_' + row_id + "_41").prop('readonly', false);  
                     }

                     if (parseFloat(jsonData.wholesale_price) !== 0) {
                        $('#td_data_' + row_id + "_42").val(jsonData.wholesale_price);
                        $('#td_data_' + row_id + "_42").prop('readonly', true);
                     }
                     else
                     {
                        $('#td_data_' + row_id + "_42").val('');
                        $('#td_data_' + row_id + "_42").prop('readonly', false);  
                     }
                     
                     if (jsonData.alphabet_price != '') {
                        $('#td_data_' + row_id + "_43").val(jsonData.alphabet_price);
                        $('#td_data_' + row_id + "_43").prop('readonly', true);
                     }
                     else
                     {
                        $('#td_data_' + row_id + "_43").val('');
                        $('#td_data_' + row_id + "_43").prop('readonly', false);  
                     }

                     if (parseFloat(jsonData.mrp_price) !== 0) {
                        $('#td_data_' + row_id + "_44").val(jsonData.mrp_price);
                        $('#td_data_' + row_id + "_44").prop('readonly', true);
                     }
                     else
                     {
                        $('#td_data_' + row_id + "_44").val('');
                        $('#td_data_' + row_id + "_44").prop('readonly', false);  
                     }
                  ...
                  // id,purchase_price,sales_price,wholesale_price,alph_price,mrp,discount

                  // setting values to grid inputs : 
      ...
      /* ---------- ON BATCH CHANGE SELECT CLOSE -------------*/

      /* ---------- check for same item with same batch is present -------------*/
      function checkForDuplicates() {
         ...
         rows.forEach((row) => {
            ...
            const batchSelect = row.querySelector("[id^='batch_id_']");

            if (!itemIdInput || !batchSelect) {
               console.warn("Missing elements in a row, skipping...");
               return;
            }
            ...
            console.log(`Checking: Item ${itemId} - Batch ${batchNo}`);
            ...
            if (itemId && batchNo) {
               ...
               if (combinations.has(key)) {
                  ...
                  // alert(`Duplicate entry detected: Item ID ${itemId} with batch ${batchNo} already exists.`);
      ...
      /* ---------- check for same item with same batch is present close -------------*/

      function purchase_price_change(i) {
         let purchasePrice = $('#td_data_' + row_id + "_4").val() || 0;
         let salesPrice = $('#td_data_' + row_id + "_41").val() || 0;
         let wholesalePrice = $('#td_data_' + row_id + "_42").val() || 0;
         let mrpPrice = $('#td_data_' + row_id + "_44").val() || 0;

         if (purchasePrice > salesPrice) {
            errorMessage = "Purchase Price must be less than or equal to Sales Price.";
         } else if (salesPrice < wholesalePrice) {
            errorMessage = "Sales Price must be greater than or equal to Wholesale Price.";
         } else if (mrpPrice < purchasePrice || mrpPrice < salesPrice || mrpPrice < wholesalePrice) {
            errorMessage = "MRP must be greater than or equal to Purchase, Sales, and Wholesale Price.";
         }
         ...
         for (i = 1; i <= rowcount; i++) {
            ...
            if (document.getElementById("td_data_" + i + "_3")) {
               ...
               if ($("#td_data_" + i + "_3").val() != null && $("#td_data_" + i + "_3").val() != '') {
                  ...
                  console.log("Inner if statement line 715 ");
               }

            } //if end
            else {
               console.log("Inner if statement line 721 ");
            }
         } //for end
      ...
      function show_batch_modal(row_id) {
         ...
         //Set to Popup
         $("#batchpop_pro_id").val(item_id);
         $("#batchpop_row_id").val(row_id);
