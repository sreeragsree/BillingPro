...
function get_sales_payment_type($sale_id)
{
  $CI = &get_instance();

  // Query to get the payment types for the given sales_id
  $CI->db->select("GROUP_CONCAT(payment_type ORDER BY payment_type SEPARATOR ',') AS payment_types");
  $CI->db->from("db_salespayments");
  $CI->db->where("sales_id", $sale_id);
  $query = $CI->db->get();

  // Check if the query is successful and return the result
  if ($query->num_rows() > 0) {
    $result = $query->row();
    return $result->payment_types;
  } else {
    return ''; // Return empty string if no payments are found
  }
}

function get_rental_payment_type($rental_id)
{
  $CI = &get_instance();

  // Query to get the payment types for the given sales_id
  $CI->db->select("GROUP_CONCAT(payment_type ORDER BY payment_type SEPARATOR ',') AS payment_types");
  $CI->db->from("db_rentalpayments");
  $CI->db->where("rental_id", $rental_id);
  $query = $CI->db->get();
...
function get_init_code($value, $store_id = '', $only_code_flag = false)
{
  ...
  if ($value == 'rental_item')
    $CI->db->select("rent_item_init");
  if ($value == 'item')
  ...
  if ($value == 'rental_payment')
    $CI->db->select("rental_payment_init");
  ...
  if ($value == 'rental_item') {
    $maxid = get_count_id('db_rentalitems');
    return $query->rent_item_init . str_pad($maxid, 4, '0', STR_PAD_LEFT);
  }
  if ($value == 'item') {
  ...
  // rental
  if ($value == 'rental') {
    $maxid = get_count_id('db_rented');
    return ($only_code_flag) ? $query->rental_init : $query->init_code . str_pad($maxid, 4, '0', STR_PAD_LEFT);
  }
  // rental close
  if ($value == 'sales_return') {
...
function total_available_qty_rental_items_of_store($warehouse_id = '', $store_id = '', $item_id = '')
{
  $CI = &get_instance();
  if (empty($store_id)) {
    $store_id = get_current_store_id();
  ...
  //   $CI->db->where("warehouse_id in ($warehouse_id)");
  // }
  $CI->db->select("COALESCE(stock,0) as available_qty")->where("store_id", $store_id)->from("db_rentalitems");
  //echo $CI->db->get_compiled_select();exit;
  return $CI->db->get()->row()->available_qty;
}
...
function get_paid_cob($customer_id)
{ //Customer Opening Balance Paid Total
  $CI = &get_instance();
...
function get_paid_sob($supplier_id)
{ //supplier Opening Balance Paid Total
  $CI = &get_instance();
  return $CI->db->select("coalesce(sum(payment),0) as payment")
    ->where("store_id", get_current_store_id())
...
function get_rental_details($sales_id)
{
  $CI = &get_instance();
  return $CI->db->select('*')->from('db_rented')->where('id', $sales_id)->get()->row();
}
...
function get_rental_item_details($item_id)
{
  $CI = &get_instance();
  return $CI->db->select("*")
    ->from("db_rentalitems")
...
function validate_subscription()
{
  ...
  if (empty($subscription_id)) {
    echo "This store don't have any subscrtions!!";
    exit;
  }

  $expire_date = get_subscription_rec($subscription_id)->expire_date;
  if ($expire_date < date('Y-m-d')) {
    echo "Store Subscription expired!!";
    exit;
  }
  return true;
}
function validate_package_offers($column, $table_name, $store_id = '')
{
  ...
  if (empty($column)) {
    echo "Missing!! Package Validation";
    exit;
  }
  ...
  if (!$sub_id && !is_admin()) {
    echo $CI->lang->line('subscription_msg_1');
    exit;
  } else {
    ...
    if ($column == 'max_invoices') {
      ...
      if ($get_subscription_rec->max_invoices != -1) {
        ...
        if (get_tot_table_rec($table_name, $store_id) >= $get_subscription_rec->max_invoices) {
          echo $CI->lang->line('max_invoices_used');
          exit;
        }
      }
    }
    if ($column == 'max_items') {
      ...
      if ($get_subscription_rec->max_items != -1) {
        ...
        if (get_tot_table_rec($table_name, $store_id) >= $get_subscription_rec->max_items) {
          echo $CI->lang->line('max_items_used');
          exit;
        }
      }
    }
    if ($column == 'max_warehouses') {
      if ($get_subscription_rec->max_warehouses != -1) {
        ...
        if (get_tot_table_rec($table_name, $store_id) >= $get_subscription_rec->max_warehouses) {
          echo $CI->lang->line('max_warehouses_used');
          exit;
        }
      }
    }
    if ($column == 'max_users') {
      if ($get_subscription_rec->max_users != -1) {
        ...
        if ((get_tot_table_rec($table_name, $store_id) - $store_admin_count) >= $get_subscription_rec->max_users) {
          echo $CI->lang->line('max_users_used');
          exit;
        }
      }
    }
...
function autosynch_rental_code()
{
  $init_code = get_only_init_code('rental');
  $count_id = get_last_count_id('db_rented');
