...
function get_customer_tot_advance($customer_id){
	...
	$advance_rental_adjusted = $CI->db->select('coalesce(sum(advance_adjusted),0) as advance_adjusted')
							->where('customer_id',$customer_id)
							->get('db_rentalpayments')->row()->advance_adjusted;

	$tot_advance -= $advance_adjusted;
	$tot_advance -= $advance_rental_adjusted;
